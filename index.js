...
const PROMO_MESSAGE = `✨ Awali bulan April dengan wangi terbaik dari My Perfume
...
📍di jalan panglegur selatan pertigaan lampu merah terminal ronggosukowati (timur jalan)`;
...
const sendWAMessage = async (phone, message) => {
    ...
    try {
        ...
        console.error(`[WA Error] Gagal ke ${formattedPhone}:`, error.response?.data || error.message);
        throw error;
    }
};
...
async function safeSend(phone, msg, retry = 2) {
    try {
        ...
        if (errorMsg.includes("not on whatsapp") || errorMsg.includes("INVALID_JID")) {
...
async function runBroadcast(limit) {
    ...
    try {
        ...
        const [rows] = await conn.execute(
            'SELECT id, hp FROM unira WHERE status = 0 LIMIT ?', 
            [limit]
        );
...
async function sendProgressReport() {
    ...
    try {
        conn = await pool.getConnection();
        const [rows] = await conn.execute(
            'SELECT status, COUNT(*) as total FROM unira GROUP BY status'
        );

        let report = { 0: 0, 1: 0, 2: 0 };
        rows.forEach(r => report[r.status] = r.total);

        const msg = `📊 *Laporan Progres My Perfume*\n\n` +
                    `✅ Berhasil: ${report[1]}\n` +
                    `❌ Gagal: ${report[2]}\n` +
...
cron.schedule('0 9 * * *', () => runBroadcast(3), { timezone: "Asia/Jakarta" });

// Jam 12 Siang - Kirim 3 pesan
cron.schedule('0 12 * * *', () => runBroadcast(3), { timezone: "Asia/Jakarta" });

// Jam 4 Sore - Kirim 3 pesan
cron.schedule('0 16 * * *', () => runBroadcast(3), { timezone: "Asia/Jakarta" });

// Jam 5 Sore - Laporan ke Admin
cron.schedule('0 17 * * *', () => sendProgressReport(), { timezone: "Asia/Jakarta" });
